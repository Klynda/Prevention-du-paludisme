...
def generateur3(N, M, pointRencontre, alpha, seuil, Qs = None, endemicites = None):
    #GENERATION DES LIEU DE RÉSIDENCES
    #initialisation nbre de zones
    n = N
    ...
    if pr != 0 :
        ...
        for i in range(pr-1):
            listInd = []
            identifiant = 'Z'+str(i+1)
            frequentation[identifiant] = input("Entrez frequentation "+identifiant+", SVP : ")
            for j in range(nbIndPr) :
                listInd.append('H'+str(k))
                residence['H'+str(k)] = identifiant
                k +=1
    ...
    for idH, idZ in residence.iteritems():
        ...
        if not(listPr.__contains__(idZ)) :
            if pd<= seuil and pr != 0 :
                ...
                #liste des points de rencontre disponibles
                listPr = range(1, pr+1, 1)
                # ICI PASSE PAR TOUS LES POINTS DE RENCONTRE
                for i in range (pr):
                    zChoisie = 'Z'+str(listPr[i])
                    prChoisis.append(zChoisie)
                    path[idH].append( (zChoisie, qs*frequentation[zChoisie]) )
                    if presences.__contains__(zChoisie) :
                        presences[zChoisie].append((idH, qs*frequentation[zChoisie]))
                    else :
                        presences[zChoisie] = [(idH, qs*frequentation[zChoisie])]
            else :
                path[idH]= [(idZ, 1)]
                if presences.__contains__(idZ) :
                    presences[idZ].append( (idH, 1) )
